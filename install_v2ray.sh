#!binbash
# v2ray一键安装脚本
# Modify by ifenghttpswww.hicairo.com
...
SITES=(
	httpswww.ruiwen.com
	httpswww.ihuaben.com
	httpwww.fbook.net
	httpswww.tadu.com
	httpwww.quyuewang.cn
	httpswww.hongshu.com
	httpwww.zongheng.com
	httpwww.cjzww.com
	httpwww.tiandizw.com
	httpsb.faloo.com
	httpswww.17k.com
	httpswww.yousuu.com
	httpwww.qwsy.com
	httpwww.inbook.net
	httpswww.zzwenxue.com
)

CONFIG_FILE=etcv2rayconfig.json
SERVICE_FILE=etcsystemdsystemv2ray.service
...
WgcfIPv4Status=$(curl -s4m8 httpswww.cloudflare.comcdn-cgitrace -k  grep warp  cut -d= -f2)
WgcfIPv6Status=$(curl -s6m8 httpswww.cloudflare.comcdn-cgitrace -k  grep warp  cut -d= -f2)
if [[ $WgcfIPv4Status =~ onplus ]]  [[ $WgcfIPv6Status =~ onplus ]]; then
    wg-quick down wgcf devnull 2&1
    systemctl stop warp-go devnull 2&1
    checkv4v6
    wg-quick up wgcf devnull 2&1
    systemctl start warp-go devnull 2&1
else
    checkv4v6
    if [[ -z $v4 && -n $v6 ]]; then
        colorEcho $GREEN 检测到为纯IPv6 VPS, 已自动添加DNS64解析服务器
        echo -e nameserver 2a014f8c2c123f1  etcresolv.conf
		IP=$v6
		ipv6Status=on
		else
		IP=$v4
...
BT=false
NGINX_CONF_PATH=etcnginxconf.d
res=`which bt 2devnull`

if [[ $res !=  ]]; then
    BT=true
    NGINX_CONF_PATH=wwwserverpanelvhostnginx
fi
...
checkSystem() {
    ...
    res=`which yum 2devnull`
    if [[ $ != 0 ]]; then
        res=`which apt 2devnull`
        if [[ $ != 0 ]]; then
...
getVersion() {
    if usrbinv2rayv2ray -version devnull 2&1;then
	VER=$(usrbinv2rayv2ray -version  awk 'NR==1 {print $2}')
    else
	VER=$(usrbinv2rayv2ray version  awk 'NR==1 {print $2}')
    fi
    RETVAL=$
    CUR_VER=$(normalizeVersion $(echo $VER  head -n 1  cut -d   -f2))
    TAG_URL=httpsapi.github.comreposv2flyv2ray-corereleaseslatest
    NEW_VER=$(normalizeVersion $(curl -s ${TAG_URL} --connect-timeout 10 tr ',' 'n'  grep 'tag_name'  cut -d -f4))
...
getData() {
    if [[ $TLS = true  $XTLS = true ]]; then
        ...
        colorEcho ${BLUE}   3. 如果root目录下有 v2ray.pem 和 v2ray.key 证书密钥文件，无需理会条件2
        echo  
        ...
        if [[ -f ~v2ray.pem && -f ~v2ray.key ]]; then
            ...
            CERT_FILE=etcv2ray${DOMAIN}.pem
            KEY_FILE=etcv2ray${DOMAIN}.key
        else
            resolve=`curl -sm8 ipget.netip=${DOMAIN}`
    ...
    if [[ $KCP = true ]]; then
        ...
        SEED=`cat procsyskernelrandomuuid`
    fi

    if [[ $TROJAN = true ]]; then
        echo 
        read -p  请设置trojan密码（不输则随机生成） PASSWORD
        [[ -z $PASSWORD ]] && PASSWORD=`cat devurandom  tr -dc 'a-zA-Z0-9'  fold -w 16  head -n 1`
    ...
    if [[ $XTLS = true ]]; then
        ...
        read -p   请选择流控模式[默认direct] answer
        [[ -z $answer ]] && answer=1
        case $answer in
            1)
    ...
    if [[ ${WS} = true ]]; then
        ...
        while true
        do
            read -p  请输入伪装路径，以开头(不懂请直接回车)： WSPATH
            if [[ -z ${WSPATH} ]]; then
                len=`shuf -i5-12 -n1`
                ws=`cat devurandom  tr -dc 'a-zA-Z0-9'  fold -w $len  head -n 1`
                WSPATH=$ws
                break
            elif [[ ${WSPATH01} !=  ]]; then
                colorEcho ${RED}   伪装路径必须以开头！
            elif [[ ${WSPATH} =  ]]; then
    ...
    if [[ $TLS = true  $XTLS = true ]]; then
        ...
        echo    1) 静态网站(位于usrsharenginxhtml)
        echo    2) 小说站(随机选择)
        echo    3) 美女站(httpwww.kimiss.com)
        echo    4) 高清壁纸站(httpswww.wallpaperstock.net)
        echo    5) 自定义反代站点(需以http或者https开头)
        read -p   请选择伪装网站类型[默认高清壁纸站] answer
        if [[ -z $answer ]]; then
            PROXY_URL=httpswww.wallpaperstock.net
        else
            case $answer in
            1)
                PROXY_URL=
            ...
            2)
                ...
                do
                    ...
                    ip=`curl -sm8 ipget.netip=${host}`
                    res=`echo -n ${ip}  grep ${host}`
                    if [[ ${res} =  ]]; then
                        echo $ip $host  etchosts
                        break
                    fi
                done
                ;;
            3)
                PROXY_URL=httpwww.kimiss.com
                ;;
            4)
                PROXY_URL=httpswww.wallpaperstock.net
                ;;
    ...
    echo 
    read -p  是否安装BBR(默认安装)[yn] NEED_BBR
...
gpgcheck=1
enabled=1
gpgkey=httpsnginx.orgkeysnginx_signing.key
module_hotfixes=true'  etcyum.repos.dnginx.repo
            fi
        fi
        $CMD_INSTALL nginx
        if [[ $ != 0 ]]; then
            colorEcho $RED  Nginx安装失败，请到 httpswww.hicairo.com 反馈
            exit 1
        fi
        systemctl enable nginx
    else
        res=`which nginx 2devnull`
        if [[ $ != 0 ]]; then
...
startNginx() {
    if [[ $BT = false ]]; then
        ...
        nginx -c wwwservernginxconfnginx.conf
    fi
}
...
getCert() {
    ...
    if [[ -z ${CERT_FILE+x} ]]; then
        ...
        curl -sL httpsget.acme.sh  sh -s email=webmaster@hicairo.com
        source ~.bashrc
        ~.acme.shacme.sh  --upgrade  --auto-upgrade
        ~.acme.shacme.sh --set-default-ca --server letsencrypt

		if [[ $ipv6Status = on ]]; then
			if [[ $BT = false ]]; then
				~.acme.shacme.sh   --issue -d $DOMAIN --keylength ec-256 --pre-hook systemctl stop nginx --post-hook systemctl restart nginx  --standalone --listen-v6 --insecure
			else
				~.acme.shacme.sh   --issue -d $DOMAIN --keylength ec-256 --pre-hook nginx -s stop  { echo -n ''; } --post-hook nginx -c wwwservernginxconfnginx.conf  { echo -n ''; }  --standalone --listen-v6 --insecure
			fi
		else
			if [[ $BT = false ]]; then
				~.acme.shacme.sh   --issue -d $DOMAIN --keylength ec-256 --pre-hook systemctl stop nginx --post-hook systemctl restart nginx  --standalone --insecure
			else
				~.acme.shacme.sh   --issue -d $DOMAIN --keylength ec-256 --pre-hook nginx -s stop  { echo -n ''; } --post-hook nginx -c wwwservernginxconfnginx.conf  { echo -n ''; }  --standalone --insecure
			fi
		fi		
		
        [[ -f ~.acme.sh${DOMAIN}_eccca.cer ]]  {
            colorEcho $RED  获取证书失败，请复制上面的红色文字到 httpswww.hicairo.com 反馈
            exit 1
        }
        KEY_FILE=etcv2ray${DOMAIN}.key
		CERT_FILE=etcv2ray${DOMAIN}.pem
        ~.acme.shacme.sh  --install-cert -d $DOMAIN --ecc 
            --key-file       $KEY_FILE  
            --fullchain-file $CERT_FILE 
            --reloadcmd     service nginx force-reload
        [[ -f $CERT_FILE && -f $KEY_FILE ]]  {
            colorEcho $RED  获取证书失败，请到 httpswww.hicairo.com 反馈
            exit 1
        }
    else
        cp ~v2ray.pem etcv2ray${DOMAIN}.pem
        cp ~v2ray.key etcv2ray${DOMAIN}.key
    fi
}

configNginx() {
    mkdir -p usrsharenginxhtml;
    if [[ $ALLOW_SPIDER = n ]]; then
        echo 'User-Agent '  usrsharenginxhtmlrobots.txt
        echo 'Disallow '  usrsharenginxhtmlrobots.txt
    ...
    if [[ $BT = false ]]; then
        if [[ ! -f etcnginxnginx.conf.bak ]]; then
            mv etcnginxnginx.conf etcnginxnginx.conf.bak
        fi
        res=`id nginx 2devnull`
        if [[ $ != 0 ]]; then
            user=www-data
        else
            user=nginx
        fi
        cat  etcnginxnginx.conf-EOF
user $user;
...
include usrsharenginxmodules.conf;
...
http {
    ...
    # Load modular configuration files from the etcnginxconf.d directory.
    # See httpnginx.orgendocsngx_core_module.html#include
...
server {
    ...
    ssl_ciphers TLS13-AES-256-GCM-SHA384TLS13-CHACHA20-POLY1305-SHA256TLS13-AES-128-GCM-SHA256TLS13-AES-128-CCM-8-SHA256TLS13-AES-128-CCM-SHA256EECDH+CHACHA20EECDH+AES128RSA+AES128EECDH+AES256RSA+AES256EECDH+3DESRSA+3DES!MD5;
    ssl_prefer_server_ciphers on;
    ssl_session_cache builtin1000 sharedSSL10m;
    ...
    location ${WSPATH} {
      proxy_redirect off;
      proxy_pass http127.0.0.1${V2PORT};
...
setSelinux() {
    if [[ -s etcselinuxconfig ]] && grep 'SELINUX=enforcing' etcselinuxconfig; then
        sed -i 'sSELINUX=enforcingSELINUX=permissiveg' etcselinuxconfig
        setenforce 0
    fi
}

setFirewall() {
    res=`which firewall-cmd 2devnull`
    if [[ $ -eq 0 ]]; then
...
installBBR() {
    ...
    if [[ $PMT = yum ]]; then
            ...
            rpm -Uvh httpwww.elrepo.orgelrepo-release-7.0-4.el7.elrepo.noarch.rpm
            $CMD_INSTALL --enablerepo=elrepo-kernel kernel-ml
            $CMD_REMOVE kernel-3.
            grub2-set-default 0
            echo tcp_bbr  etcmodules-load.dmodules.conf
            INSTALL_BBR=true
    else
...
installV2ray() {
    rm -rf tmpv2ray
    mkdir -p tmpv2ray
    DOWNLOAD_LINK=httpsgithub.comv2flyv2ray-corereleasesdownload${NEW_VER}v2ray-linux-$(archAffix).zip
    colorEcho $BLUE  下载V2Ray ${DOWNLOAD_LINK}
    curl -L -H Cache-Control no-cache -o tmpv2rayv2ray.zip ${DOWNLOAD_LINK}
    if [ $ != 0 ];then
        colorEcho $RED  下载V2ray文件失败，请检查服务器网络设置
        exit 1
    fi
    v2ray_start_config=run -c
    mkdir -p 'etcv2ray' 'varlogv2ray' && 
    unzip tmpv2rayv2ray.zip -d tmpv2ray
    mkdir -p usrbinv2ray
    cp tmpv2rayv2ray usrbinv2ray; cp tmpv2raygeo usrbinv2ray;
    chmod +x 'usrbinv2rayv2ray'  {
    colorEcho $RED  V2ray安装失败
    exit 1
    }
    if [[ $NEW_VER = v4.32.1 ]]; then
	cp tmpv2rayv2ctl usrbinv2ray;
	chmod +x 'usrbinv2rayv2ctl'  {
        colorEcho $RED  V2ray安装失败
        exit 1
	}
...
Documentation=httpswww.v2fly.org
...
# More discussion at httpsgithub.comv2rayv2ray-coreissues1011
User=root
#User=nobody
NoNewPrivileges=true
ExecStart=usrbinv2rayv2ray $v2ray_start_config etcv2rayconfig.json
Restart=on-failure
...
vmessConfig() {
    local uuid=$(cat 'procsyskernelrandomuuid')
...
uninstall() {
    ...
    if [[ ${answer,,} = y ]]; then
        ...
        if [[ $BT = false ]]; then
            ...
            rm -rf etcnginxnginx.conf
            if [[ -f etcnginxnginx.conf.bak ]]; then
                mv etcnginxnginx.conf.bak etcnginxnginx.conf
            fi
        fi
        if [[ $domain !=  ]]; then
            rm -rf $NGINX_CONF_PATH${domain}.conf
        fi
        [[ -f ~.acme.shacme.sh ]] && ~.acme.shacme.sh --uninstall
...
outputTrojan() {
    if [[ $xtls = true ]]; then
        echo -e    ${BLUE}IP域名(address) ${PLAIN} ${RED}${domain}${PLAIN}
        echo -e    ${BLUE}端口(port)：${PLAIN}${RED}${port}${PLAIN}
...
outputVmessTLS() {
  ...
  tlstls
    ...
    echo -e    ${BLUE}传输协议(network)：${PLAIN} ${RED}${network}${PLAIN} 
    echo -e    ${BLUE}伪装域名主机名(host)SNIpeer名称：${PLAIN}${RED}${domain}${PLAIN}
    echo -e    ${BLUE}底层安全传输(tls)：${PLAIN}${RED}TLS${PLAIN}
    echo  
    echo -e    ${BLUE}vmess链接 ${PLAIN}$RED$link$PLAIN
}
...
menu() {
    ...
    echo -e # ${GREEN}网址${PLAIN} httpswww.hicairo.com                             #
    echo -e # ${GREEN}TG群${PLAIN} httpst.meHiaiFeng                               #
    echo -e #                                                           #
...
[[ -z $1 ]] && action=menu
case $action in
    menuupdateuninstallstartrestartstopshowInfoshowLog)
        ${action}
        ;;
    )
        echo  参数错误
        echo  用法 `basename $0` [menuupdateuninstallstartrestartstopshowInfoshowLog]
        ;;
esac
